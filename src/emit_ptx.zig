...
fn canonicalizeMLIRFile(allocator: std.mem.Allocator, input_file: []const u8, verbose: bool) ![]const u8 {
    ...
    if (verbose) {
        ...
        std.debug.print("   🎯 Used MLIR C API for: parse → canonicalize → GPU kernel outlining → SCF to CF → GPU to NVVM (bare ptr) → NVVM to LLVM → finalize MemRef to LLVM → convert Func to LLVM (bare ptr) → reconcile unrealized casts → output\n", .{});
