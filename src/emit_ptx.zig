...
pub fn main() !void {
    ...
    // Generate PTX header
    const header = try std.fmt.allocPrint(allocator, "// Generated PTX for function: {s}\n// Target: SM {d}\n// Generated by DCC emit_ptx tool using fully integrated MLIR C API pipeline\n\n", .{ args.function_name, args.sm_version });
    defer allocator.free(header);
...
fn generatePTXFromStatic(allocator: std.mem.Allocator, args: Args) ![]const u8 {
    ...
    // Steps 1-7: Canonicalize, GPU kernel outlining, SCF to CF, GPU to NVVM, NVVM to LLVM, finalize MemRef, convert Func, and reconcile unrealized casts using MLIR library
    ...
    // Steps 1-7 are now fully integrated above with our custom C wrapper and MLIR C API
    ...
    // Step 10: Translate MLIR to LLVM IR
    if (args.verbose) std.debug.print("🔄 Translating MLIR to LLVM IR...\n", .{});

    // Write the fixed MLIR to a temporary file for mlir-translate
    const temp_mlir_file = "temp_standalone_fixed.mlir";
    std.fs.cwd().writeFile(.{ .sub_path = temp_mlir_file, .data = fixed_mlir }) catch |err| {
        std.debug.print("Error writing temp MLIR file: {}\n", .{err});
        return EmitPtxError.PipelineError;
    };
    defer std.fs.cwd().deleteFile(temp_mlir_file) catch {};

    try runCommand(allocator, &[_][]const u8{ "mlir-translate", temp_mlir_file, "--mlir-to-llvmir", "-o", "kernel.ll" });
