...
pub fn main() !void {
    ...
    // Generate PTX header
    const header = try std.fmt.allocPrint(allocator, "// Generated PTX for function: {s}\n// Target: SM {d}\n// Generated by DCC emit_ptx tool using integrated MLIR C API pipeline\n\n", .{ args.function_name, args.sm_version });
    defer allocator.free(header);
...
fn generatePTXFromStatic(allocator: std.mem.Allocator, args: Args) ![]const u8 {
    ...
    // Steps 1-4: Canonicalize, GPU kernel outlining, SCF to CF, and GPU to NVVM using MLIR library
    if (args.verbose) std.debug.print("📝 Running integrated MLIR passes (canonicalize, GPU kernel outlining, SCF to CF, GPU to NVVM with bare ptr)...\n", .{});
    try canonicalizeMLIRFile(allocator, "simple_vector_add_gpu.mlir", "step4_nvvm.mlir", args.verbose);

    // Step 4 is now integrated above with our custom C wrapper
...
fn canonicalizeMLIRFile(allocator: std.mem.Allocator, input_file: []const u8, output_file: []const u8, verbose: bool) !void {
    if (verbose) {
        std.debug.print("🔧 Steps 1-4: Integrated MLIR passes using C API\n", .{});
    }
    ...
    if (verbose) {
        std.debug.print("✅ Created pass manager with integrated passes: canonicalize, GPU kernel outlining, SCF to CF, GPU to NVVM (bare ptr)\n", .{});
    }
    ...
    if (verbose) {
        ...
        std.debug.print("   🎯 Used MLIR library for: parse → canonicalize → GPU kernel outlining → SCF to CF → GPU to NVVM (bare ptr) → output\n", .{});
