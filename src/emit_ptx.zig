...
pub fn main() !void {
    ...
    // Generate GPU function using MLIR
    mlir.generateGpuFunction(func_decl) catch |err| {
        std.debug.print("Error generating GPU function: {}\n", .{err});
        return EmitPtxError.MlirError;
    };

    // Convert MLIR to PTX
    const ptx_code = mlir.lowerMLIRToPTX() catch |err| {
        std.debug.print("Error lowering to PTX: {}\n", .{err});
        return EmitPtxError.MlirError;
    };
    ...
    std.debug.print("// Generated by DCC emit_ptx tool using MLIR\n\n", .{});
    std.debug.print("{s}\n", .{ptx_code});
}
