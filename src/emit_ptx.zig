...
pub fn main() !void {
    ...
    // Generate PTX header
    const header = try std.fmt.allocPrint(allocator, "// Generated PTX for function: {s}\n// Target: SM {d}\n// Generated by DCC emit_ptx tool using fully integrated MLIR C API pipeline (no external mlir-opt calls)\n\n", .{ args.function_name, args.sm_version });
    defer allocator.free(header);
...
fn generatePTXFromStatic(allocator: std.mem.Allocator, args: Args) ![]const u8 {
    ...
    try canonicalizeMLIRFile(allocator, "simple_vector_add_gpu.mlir", "step7_final.mlir", args.verbose);
    ...
    try extractKernelFunction(allocator, "step7_final.mlir", "standalone_kernel.mlir");
    ...
    if (!args.verbose) {
        std.fs.cwd().deleteFile("step7_final.mlir") catch {};
