...
test "type system - tensor simple" {
    ...
    {
        const file = try std.fs.cwd().createFile("test_tensor.toy", .{ .truncate = true });
        defer file.close();
        try file.writeAll(test_source);
    }
    defer std.fs.cwd().deleteFile("test_tensor.toy") catch {};

    const out = try process.Child.run(.{ .allocator = allocator, .argv = &.{ dcc_path, "test_tensor.toy" } });
    defer allocator.free(out.stdout);
    defer allocator.free(out.stderr);

    if (out.term.Exited != 0) {
        std.debug.print("Expected compilation to work\n", .{});
        std.debug.print("{s}\n", .{out.stdout});
        std.debug.print("{s}\n", .{out.stderr});
        return error.UnexpectedExitCode;
    }
